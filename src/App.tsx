...
import { FloatNotice } from './components'
...
const componentSource = `import { Toggle } from './components'\n\n<Toggle checked={enabled} onCheckedChange={setEnabled} label="通知を受け取る" />`
const portableImports: Record<string, { name: string; usage: string }> = {
  Toggle: { name: 'Toggle', usage: '<Toggle checked={enabled} onCheckedChange={setEnabled} label="通知を受け取る" />' },
  Avatar: { name: 'Avatar', usage: '<Avatar glyph="N" color="#397f65" size={44} online />' },
  OfficialBadge: { name: 'OfficialBadge', usage: '<OfficialBadge />' },
  PremiumBadge: { name: 'PremiumBadge', usage: '<PremiumBadge size={24} />' },
  FloatNotice: { name: 'FloatNotice', usage: '<FloatNotice>保存しました</FloatNotice>' },
  MessageInput: { name: 'MessageComposer', usage: '<MessageComposer value={draft} onValueChange={setDraft} onSend={send} />' },
  'Reply preview': { name: 'MessageComposer', usage: '<MessageComposer value={draft} onValueChange={setDraft} onSend={send} reply={{ author: "ねずみさん", text: "返信元" }} onDismissReply={clearReply} />' },
  PlusMenu: { name: 'MessageComposer', usage: '<MessageComposer value={draft} onValueChange={setDraft} onSend={send} menuItems={menuItems} />' },
}
...
function UiDetail({ item, onBack }: { item: UiInventoryItem; onBack: () => void }) {
  const sourceUrl = `https://github.com/nezumi0627/Vyline/blob/main/Vyline/apps/desktop/${item.path}`
  const portable = portableImports[item.title]
  const snippet = portable ? `import { ${portable.name} } from './components'\n\n${portable.usage}` : ''
